...
class espMqttClient : public MqttClientSetup<espMqttClient> {
 public:
  ...
  explicit espMqttClient(bool internalTask, uint8_t priority = 1, uint8_t core = 1);
...
class espMqttClientSecure : public MqttClientSetup<espMqttClientSecure> {
 public:
  #if defined(ARDUINO_ARCH_ESP32)
  explicit espMqttClientSecure(bool internalTask, uint8_t priority = 1, uint8_t core = 1);
